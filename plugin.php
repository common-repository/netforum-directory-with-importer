  ...
  if ( ! function_exists( 'ndwi_fs' ) ) {
    ...
    function ndwi_fs() {
        ...
        if ( ! isset( $ndwi_fs ) ) {
            ...
            $ndwi_fs = fs_dynamic_init( array(
                'id'                  => '5081',
                'slug'                => 'netforum-directory-with-importer',
                'type'                => 'plugin',
                'public_key'          => 'pk_434d8098a9470709bbdc825399cd6',
                'is_premium'          => true,
                'is_premium_only'     => true,
                'has_addons'          => false,
                'has_paid_plans'      => true,
                'menu'                => array(
                    'slug'           => 'fs_netforum_plugin_import',
                    'override_exact' => true,
                    'contact'        => false,
                    'support'        => false,
                    'parent'         => array(
                ...
                'secret_key'          => 'sk_U~@Qw%t1_=VwIR4p%EG-{U*U(Yh4:',
            ) );
        }
    ...
    do_action( 'ndwi_fs_loaded' );

    function ndwi_fs_settings_url() {
        return admin_url( 'tools.php?page=fs_netforum_plugin_import' );
    }

    ndwi_fs()->add_filter('connect_url', 'ndwi_fs_settings_url');
    ndwi_fs()->add_filter('after_skip_url', 'ndwi_fs_settings_url');
    ndwi_fs()->add_filter('after_connect_url', 'ndwi_fs_settings_url');
    ndwi_fs()->add_filter('after_pending_connect_url', 'ndwi_fs_settings_url');
}
...
class fs_netforum_plugin {
    ...
    function javascript_table_load() {
        ...
        if (is_a($post, 'WP_Post') && (has_shortcode($post->post_content, self::$table_shortcode) || has_shortcode($post->post_content, self::$json_shortcode))) {
            ...
            wp_enqueue_script('fsnet_dataTablesMin', plugins_url('js/jquery.dataTables.js', __FILE__));
    ...
    function add_jquery_ui($hook) {
        if (strpos($hook, 'fs_netforum_plugin') === false) return;
    ...
    function json_table($atts) {
        ...
        if ($short_attrs['limit'] < 0 && $short_attrs['display_members_only'] == 0) {
            ...
            $results = $wpdb->get_results($wpdb->prepare
                            ("SELECT " . $rows_to_display . " FROM {$wpdb->prefix}fsnet_user_master WHERE {$do_not_disp} (is_member = 1 OR recv_benefits = 1) LIMIT %d", $short_attrs['limit']));
        }
...
';
    ...
    function fs_plugin_menu() {
        add_management_page("Netforum Directory", "Netforum Directory", "manage_options", "fs_netforum_plugin_import", array($this,"generate_plugin_page"));
    }
    ...
    function generate_plugin_page() {
        ...
        if (isset($_POST['process_import'])) {
            $nonce = wp_create_nonce( 'process-import' );
            $this->process_import($nonce);
        } else {
            $nonce = wp_create_nonce( 'display-import-page' );
            $this->display_import_page($nonce);
        }
    }

    private function display_import_page($nonce) {
        
        if ( ! wp_verify_nonce( $nonce, 'display-import-page' ) ) {
            echo '<div class="updated"><p><strong>Security Issue.</strong></p></div>';
            return;
        } else {
        ?>
        <script>
            ...
            jQuery(function () {
                jQuery("#import_frm").submit(function (event) {
                    var data = "";
    ...
    private function process_import($nonce) {
        //import here
        
        if ( ! wp_verify_nonce( $nonce, 'process-import' ) ) {
            echo '<div class="updated"><p><strong>Security Issue.</strong></p></div>';
            return;
        } else {
            global $wpdb;
            ...
            $nonce = wp_create_nonce( 'process-file-in' );

            $local_path = $this->process_file_in($nonce);
            if ($local_path === -1) {
            ...
            $process_import = sanitize_text_field($_POST['process_import']);
            $input_vals = explode(",", $process_import);
    ...
    private function process_file_in($nonce) {
        
        if( !wp_verify_nonce( $nonce, 'process-file-in' ) ){
